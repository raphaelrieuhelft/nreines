...
\noindent
$n_{x,c}$ est le nombre de reines sur la cellule $c$ dans la configuration $x$, et $s_{x,c} = (s_{x,c,N}, s_{x,c,NE}, s_{x,c,E}, \ldots, s_{x,c,NW})$ code la présence ou l'absence de signaux venant de chaque direction : $s_{x,c,d}$ vaut $1$ si dans la configuration $x$, la cellule $c$ contient un signal provenant de la direction $d$.
...
\]}
 ...
 \delta(c_{from}, c_{to}) =
  ...
  \end{cases}
\] }
  \item{On peut alors définir $\M :{\Ne}\to\{0,1\}$ telle que $\M(c_{from}, c_{to})$ vaut $1$ si une reine se déplace de $c_{from}$ à $c_{to}$ et $0$ sinon. Formellement : \[
    \M(c_{from}, c_{to}) = 
    \begin{cases}
      \hfill 0 \hfill & \text{ si $\Ss(c_{from})=1$} \\
      \hfill 1 \hfill & \text{ si : }{ \begin{cases} \Ss(c_{from})=0 \\\text{et $n_{c_{from}}\geq 1$} \\ \text{et } \Sg(c_{to}, \delta(c_{from}, c_{to}))=1\\ \end{cases}}\\
