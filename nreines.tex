...
La procédure de mise à jour de deux cases adjacentes $c$  et $c'$
%, avec $\norm{\begin{pmatrix}i-i'\\j-j'\end{pmatrix}}_\infty=1$,
 %en notant $d_{1\rightarrow 2}$ la direction de $c$ vers $c'$ et $d_{2\rightarrow 1}$ la direction opposée,
...
Définissons d'abord quelques fonctions auxiliaires. 
 ...
 \Ss(c) =
  ...
  \end{cases}
\]}
  \item{On pose $\Sg :  {\La\times\D}\to\{0,1\}$ qui indique si une reine peut se déplacer vers une case en venant d'une certaine direction : \[
 \Sg(c, d_{\mathrm {from}}) =
  \begin{cases} 
