...
\title{Le problème des $\boldsymbol n$ reines}
\date{}
\author{}
\maketitle
...
On se donne deux cases adjacentes $c$ et $c'$ à mettre à jour. On commence par envisager des déplacements de reine entre $c$ et $c'$ de la façon suivante :  
\begin{itemize}
\item{ Si une reine est présente en case $c$, alors que la case $c$ contient un signal, elle est \emph{a priori} menacée : elle envisage alors de se déplacer vers la case $c'$. De même, si plusieurs reines sont présentes en $c$, elles sont menacées et l'une d'elles tente de se déplacer. S'il n'y a pas de signal en $c'$ sauf éventuellement un venant de $c$ (qui est peut-être dû à la présence de la reine qui cherche à bouger), et qu'il n'y a pas d'autre reine en $c'$, la case est considérée comme sûre et le déplacement a lieu (cf. Figure $2$). Sinon, la case est dangereuse, et le déplacement a lieu seulement avec une probabilité $\varepsilon$.
% Si le déplacement réussit, on retire une reine à la case $c$ et on en ajoute une à la case $(i', j')$. 
}

\item{ Symétriquement, une reine de $c'$ peut essayer de se déplacer vers $c$ selon la même procédure. Les deux déplacements se font de manière simultanée afin que l'ordre des deux cases choisies pour la mise à jour n'ait pas d'importance.}
...
\]}
 ...
 \Sg(c, d_{\mathrm {from}}) =
  ...
  \end{cases}
\]
 En toute rigueur, $\Ss$ et $\Sg$ devraient également prendre la configuration en argument : on l'omet pour plus de lisibilité.
}
\medskip
...
Après avoir effectué d'éventuels déplacements de reine, on met à jour les signaux de $c$ et $c'$ de la façon suivante :
